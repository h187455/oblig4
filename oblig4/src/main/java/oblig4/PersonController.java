...
public class PersonController {
    ...
    public String deltagerliste (Model model,
    	...
    	if(!LoginUtil.erBrukerInnloget(request)) {
    ...
    public String validerOgRegistrer(
        ...
        if(deltagerRepository.existsById(nyDeltager.getMobilnummer())) {
        	model.addAttribute("feilmeldinger", List.of("Mobilnumer eksister allerede.")); 
        	return "paamelding_med_melding"; 
        }
        
        if(!validatorService.isValidPassord(passord)) {
        	model.addAttribute("feilmeldinger", List.of("passord må vøre minst 8 tegn langt."));
        	return "paamelding_med_melding";
        }
    ...
    public String postMethodName(
    	...
    	if (!validatorService.isValidmoblinummer(mobilnummer)) {
    		model.addAttribute("feilmeldinger", List.of("Mobilnummer må vøre 8 stffer langt.")); 
    		return "login"; 
    	}
    	
    	if(!validatorService.isValidPassord(passord)) {
    		model.addAttribute("feilmeldinger", List.of("Passord må være mins 8 tegn langt.")); 
    		return "login"; 
    	}
